...
the main function is to decrypt tweets
when it is ready to work, sends a message to parent process though a pipe
reads encrypt file name and decrypt file name from another pipe through which parent process sends messages
when it detects parent process closed the pipe it receives data from, it closes the pipe it write data into, and return
*/
...
schedule according to the input scheduling algorithm
...
int main(int argc, char **argv){
	...
	ssize_t wlen;//return value of write()
		...
		while(EOF!=fscanf(fp,"%s",encfile)){
			...
			if(rtv==-1){//error select()
				...
				if(strcmp(feedback,CHILD_READY)==0){//child is ready
					...
					wlen = write(pipepfd[currentchild][1],encfile,MAXLENGTH);
					if(wlen<0){//error calling write()
						printf("[%s] Process ID #%d failed to write into pipe.\n",getcurtime(),getpid());
						terminateval = 1;
						goto terminate_with_children;
					}
					wlen = write(pipepfd[currentchild][1],decfile,MAXLENGTH);
					if(wlen<0){
						printf("[%s] Process ID #%d failed to write into pipe.\n",getcurtime(),getpid());
						terminateval = 1;
						goto terminate_with_children;
		...
		while(EOF != fscanf(fp,"%s",encfile)){
			...
			if(rtv==-1){//error occurred when calling select
				...
				if(strcmp(feedback,CHILD_READY)==0){//child is ready
					...
					wlen = write(pipepfd[i][1],encfile,MAXLENGTH);
					if(wlen<0){//error calling write()
						printf("[%s] Process ID #%d failed to write into pipe.\n",getcurtime(),getpid());
						terminateval = 1;
						goto terminate_with_children;
					}
					wlen = write(pipepfd[i][1],decfile,MAXLENGTH);
					if(wlen<0){//error calling write()
						printf("[%s] Process ID #%d failed to write into pipe.\n",getcurtime(),getpid());
						terminateval = 1;
						goto terminate_with_children;
